...
public enum MooShellIntegration {
    ...
    /// Adds Moo variables and, for a supported interactive shell, updates
    ...
    public static func configure(
        ...
        set("MOO_SHELL_FEATURES", shellFeatures, in: &result.environment)

        guard let resourcesDirectory else { return result }
        let resourcesPath = resourcesDirectory.standardizedFileURL.path
        set("MOO_RESOURCES_DIR", resourcesPath, in: &result.environment)
        ...
        case "nu":
            ...
            if supportsNushellInjection(arguments: result.args) {
                result.args.insert(contentsOf: ["--execute", "use moo *"], at: 0)
            }
        case "zsh":
            configureZsh(
                &result.environment,
    ...
    private static func configureBash(
        ...
        let script = resourcesPath + "/shell-integration/bash/moo.bash"
        ...
        while index < parameters.args.endIndex {
            ...
            if argument == "--norc" || argument == "--noprofile" {
                ...
                set("MOO_BASH_RCFILE", parameters.args[valueIndex], in: &parameters.environment)
                index = valueIndex
            } else {
        ...
        if let oldEnvironment = value(for: "ENV", in: parameters.environment)
            ...
            set("MOO_BASH_ENV", oldEnvironment, in: &parameters.environment)
        }
        set("ENV", script, in: &parameters.environment)
        set("MOO_BASH_INJECT", injectionFlags.joined(separator: " "), in: &parameters.environment)
        ...
        if value(for: "HISTFILE", in: parameters.environment) == nil {
            if let historyFile = processEnvironment["HISTFILE"] {
                ...
                set("MOO_BASH_UNEXPORT_HISTFILE", "1", in: &parameters.environment)
            }
        }
    ...
    private static func configureXDG(
        ...
        let newValue = ([integrationPath] + paths.filter { $0 != integrationPath })
            .joined(separator: ":")
        set("MOO_SHELL_INTEGRATION_XDG_DIR", integrationPath, in: &environment)
    ...
    private static func configureZsh(
        ...
        if let oldDirectory = value(for: "ZDOTDIR", in: environment)
            ...
            set("MOO_ZSH_ZDOTDIR", oldDirectory, in: &environment)
        }
