    ...
    func migrate(profiles: ProfileStore, windowGroups: WindowGroupStore) {
        ...
        guard sourceVersion <= Self.currentVersion else {
            issueCenter.report(PersistenceIssue(
                ...
                message: "The preferences use schema version \(sourceVersion). This version of Moo supports up to version \(Self.currentVersion).",
        ...
        if defaults.synchronize() {
            ...
            issueCenter.report(PersistenceIssue(
                ...
                sourceURL: sourceURL,
                kind: .writeFailed,
                message: "Moo could not save the migrated preferences.",
                foundVersion: sourceVersion,
                supportedVersion: Self.currentVersion
            ))
        }
    ...
    private func reconnectProfileStorage(sourceURL: URL, migratePreferences: Bool = true) {
        do {
            ...
            issueCenter.report(PersistenceIssue(
                ...
                sourceURL: sourceURL,
                kind: .unreadable,
                message: "Moo still cannot open profile storage: \(error.localizedDescription)"
            ))
        }
    }
