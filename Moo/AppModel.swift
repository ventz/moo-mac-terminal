    ...
    private init() {
        ...
        do {
            ...
            let fallback = FileManager.default.temporaryDirectory
                .appendingPathComponent("Moo-profiles-fallback-\(UUID().uuidString)")
            profiles = try! ProfileStore(
                directory: fallback,
                issueCenter: issueCenter,
                backupDirectory: backups
            ...
            issueCenter.report(PersistenceIssue(
                ...
                kind: .writeFailed,
                message: "Moo could not open profile storage. Built-in defaults are active: \(error.localizedDescription)"
            ))
        }
        themes = ThemeStore(
    ...
    private static func migrateLegacyApplicationSupport(
        ...
        do {
            ...
            return PersistenceIssue(
                ...
                message: "Moo could not copy the legacy application data: \(error.localizedDescription)"
            )
        }
    }
